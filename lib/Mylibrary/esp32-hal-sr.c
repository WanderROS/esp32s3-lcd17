...
// 唤醒词和命令词语言可通过 sr_config.h 配置
#if __has_include("sr_config.h")
  #include "sr_config.h"
#endif
#ifndef SR_WAKEWORD_NAME
  #define SR_WAKEWORD_NAME  "hiesp"
#endif
#ifndef SR_MN_LANGUAGE
...
esp_err_t sr_start(
  ...
  if (models) {
    for (int i = 0; i < models->num; i++) {
      log_i("available model[%d]: %s", i, models->model_name[i]);
    }
  }
  ...
  char *wn_name = esp_srmodel_filter(models, ESP_WN_PREFIX, SR_WAKEWORD_NAME);
  log_i("wakenet filter result: %s", wn_name ? wn_name : "NULL");
  ESP_GOTO_ON_FALSE(NULL != wn_name, ESP_ERR_NOT_FOUND, err,
                    "Wakeword model '%s' not found in srmodels.bin", SR_WAKEWORD_NAME);
  afe_config.wakenet_model_name = wn_name;
  afe_config.aec_init = false;
  log_i("load wakenet '%s'", afe_config.wakenet_model_name);
  g_sr_data->afe_data = g_sr_data->afe_handle->create_from_config(&afe_config);
  ESP_GOTO_ON_FALSE(NULL != g_sr_data->afe_data, ESP_FAIL, err, "Failed to create AFE");

  // Load Custom Command Detection
  char *mn_name = esp_srmodel_filter(models, ESP_MN_PREFIX, SR_MN_LANGUAGE);
  log_i("multinet filter result: %s", mn_name ? mn_name : "NULL");
  ESP_GOTO_ON_FALSE(NULL != mn_name, ESP_ERR_NOT_FOUND, err,
                    "Multinet model for language '%s' not found in srmodels.bin", SR_MN_LANGUAGE);
  log_i("load multinet '%s'", mn_name);
  g_sr_data->multinet = esp_mn_handle_from_name(mn_name);
  ESP_GOTO_ON_FALSE(NULL != g_sr_data->multinet, ESP_FAIL, err, "Failed to get multinet handle");
  log_i("load model_data '%s'", mn_name);
  g_sr_data->model_data = g_sr_data->multinet->create(mn_name, 5760);
  ESP_GOTO_ON_FALSE(NULL != g_sr_data->model_data, ESP_FAIL, err, "Failed to create model_data");
